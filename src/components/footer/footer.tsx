...
export function Footer() {
  ...
  return (
    <footer className="relative z-10 mt-10 flex w-full flex-col items-center justify-center gap-1 rounded-sm border border-b-0 border-r-background bg-background pb-2 pt-5 text-muted-foreground sm:max-w-[calc(100vw-212.5px)]">
      <p className="flex items-center justify-center font-mono text-xs tracking-tighter sm:translate-x-[106.25px]">
        <span>{t.footer['This site was developed with']}</span>
        <Heart className="mx-1 h-3 w-3 fill-destructive text-destructive" />
        <span>{t.footer['by Isaque Delfino']}</span>
      </p>
